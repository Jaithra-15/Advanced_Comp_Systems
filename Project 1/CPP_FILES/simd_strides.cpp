...
// --- add near your includes if not present ---
#include <set>

// --- helper: build stride set ---
static inline vector<size_t> default_strides() {
    // sweep powers of two; include 1 for unit stride equivalence
    return {1,2,4,8,16,32,64,128,256,512};
}

// --- helper: log-spaced N list (or single N if --N provided) ---
static inline vector<size_t> build_Ns(size_t N_cli, bool use_single, size_t Nmin=1000, size_t Nmax=15000000, int steps=24) {
    vector<size_t> Ns;
    if (use_single) { Ns.push_back(N_cli); return Ns; }
    double lmin = log10(double(Nmin)), lmax = log10(double(Nmax));
    for (int i=0;i<steps;i++){
        double ln = lmin + (lmax-lmin)*double(i)/double(steps-1);
        size_t N = size_t(llround(pow(10.0, ln)));
        if (Ns.empty() || N!=Ns.back()) Ns.push_back(N);
    }
    return Ns;
}

// --- OPTIONAL: detect "mode" label for CSV ---
static inline const char* mode_label(bool is_simd_build) {
#if defined(_OPENMP)
    // We only add -fopenmp-simd in your SIMD build
    return is_simd_build ? "simd" : "unknown";
#else
    return "scalar";
#endif
}

// ========================= DRIVER: STRIDE/GATHER SWEEP =========================
int main(int argc, char** argv) {
    ios::sync_with_stdio(false);
    cin.tie(nullptr);

    Config cfg = parse_cli(argc, argv);           // uses your existing CLI
    const string kernel = cfg.kernel;             // choose one kernel via --kernel=saxpy|elemmul|stencil

    // Decide N list: if user provided --N, use that single N; else log sweep
    bool use_single_N = false;
    for (int i=1;i<argc;i++) if (strncmp(argv[i],"--N=",4)==0) use_single_N = true;
    vector<size_t> Ns = build_Ns(cfg.N, use_single_N);

    // Strides to test (unit/strided/gather). For unit, we'll force stride=1.
    const vector<size_t> STRIDES = default_strides();



    ofstream ofs("stride_gather_sweep.csv");
    if (!ofs.is_open()) { cerr << "Error: cannot open stride_gather_sweep.csv\n"; return 1; }

    ofs.setf(std::ios::fixed);
    ofs.precision(6);
    ofs << "mode,kernel,dtype,access,stride,N,trial,GFLOPs,GiBps,CPE\n";

    auto run_and_write = [&](auto tag_dtype) {
        using T = decltype(tag_dtype);
        for (auto N : Ns) {
            cerr<< "iterate: " << N << endl;
            // --- UNIT-STRIDE ---
            {
                Config c = cfg; c.access = Access::Unit; c.stride = 1; c.N = N;
                auto runs = run_one<T>(c);
                for (int t=0; t<(int)runs.size(); ++t) {
                    ofs << mode_label(true) << ","  // label "simd" if built with -fopenmp-simd else "scalar"
                        << kernel << "," 
                        << (std::is_same<T,float>::value ? "f32" : (std::is_same<T,double>::value ? "f64" : "i32")) << ","
                        << "unit" << "," << 1 << "," << N << "," << t << ","
                        << runs[t].gflops << "," << runs[t].gibs << "," << runs[t].cpe << "\n";
                }
            }

            // --- STRIDED ---
            for (auto s : STRIDES) {
                if (s == 1) continue; // already covered by unit
                Config c = cfg; c.access = Access::Strided; c.stride = s; c.N = N;
                auto runs = run_one<T>(c);
                for (int t=0; t<(int)runs.size(); ++t) {
                    ofs << mode_label(true) << ","
                        << kernel << ","
            ...
            // --- GATHER ---
            for (auto s : STRIDES) {
                Config c = cfg; c.access = Access::Gather; c.stride = s; c.N = N;
                auto runs = run_one<T>(c);
                for (int t=0; t<(int)runs.size(); ++t) {
                    ofs << mode_label(true) << ","
                        << kernel << ","
    ...
    switch (cfg.dtype) {
        case DType::f32: run_and_write(float{});   break;
        case DType::f64: run_and_write(double{});  break;
        case DType::i32: run_and_write(int32_t{}); break;
    }

    ofs.close();
    cerr << "Wrote stride_gather_sweep.csv (unit, strided, gather across strides & N)\n";
    return 0;
}
