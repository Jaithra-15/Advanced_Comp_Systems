...
struct Config {
    ...
    int trials = 7;             
    double cpu_ghz = 1.5;         // changes based on the pinned clock speed
...
int main(int argc, char** argv) {
    ...
    ofstream ofs("full_sweep.csv");
    if (!ofs.is_open()) {
        cerr << "Error: could not open full_sweep.csv\n";
        return 1;
    }
    ...
    vector<DType> dtypes   = {DType::f32, DType::f64, DType::i32};
    ...
    for (const auto& k : kernels) {
        ...
        for (auto dt : dtypes) {
            ...
            for (int mis = 0; mis <= 1; ++mis) {
                cerr << "misaligned: " << mis << endl;
                for (int tail = 0; tail <= 1; ++tail) {
                    cerr << "tail_multiple: " << tail << endl;
                    cfg.misaligned = mis;
    ...
    ofs.close();
    cerr << "Wrote full_sweep.csv across all kernels × dtypes × 4 variants (misalign × tail)\n";
    return 0;
}
