...
int main(int argc, char** argv) {
    ...
    vector<string> kernels = {"saxpy", "elemmul", "stencil"};

    auto dump = [&](ofstream& ofs, const string& dtype_name,
                    const string& kernel, const vector<Metrics>& runs) {
        ofs.setf(std::ios::fixed);
        ofs.precision(6);

        // header
        ofs << "kernel,dtype,access,stride,misaligned,tail_multiple,N,trial,"
               "time_s,GFLOPs,GiBps,ns_per_elem,cpe,arith_intensity,checksum\n";

        for (int t = 0; t < (int)runs.size(); ++t) {
            const Metrics& m = runs[t];
            ofs << kernel << "," << dtype_name << ","
                << (cfg.access==Access::Unit?"unit":(cfg.access==Access::Strided?"strided":"gather")) << ","
                << cfg.stride << "," << (cfg.misaligned?1:0) << ","
                << (cfg.tail_multiple?1:0) << ","
                << cfg.N << "," << t << ","
                << m.time_s << "," << m.gflops << "," << m.gibs << ","
                << m.ns_per_elem << "," << m.cpe << "," << m.arith_intensity << ","
                << m.checksum << "\n";
        }
    };
    ...
    for (auto& k : kernels) {
        cfg.kernel = k;
        string fname = string("standard_") + string(to_string(cfg.N)) + "_" + k + string("_results.csv");
        ofstream ofs(fname);
        if (!ofs.is_open()) {
        ...
        switch (cfg.dtype) {
            case DType::f32: dump(ofs, "f32", k, run_one<float>(cfg)); break;
            case DType::f64: dump(ofs, "f64", k, run_one<double>(cfg)); break;
            case DType::i32: dump(ofs, "i32", k, run_one<int32_t>(cfg)); break;
        }
