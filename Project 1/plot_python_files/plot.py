...
files = glob.glob(os.path.join(base, "standard_*_*_results.csv"))

df_list = []
for f in files:
    # Expect filename like: standard_40960_saxpy_results.csv
    m = re.match(r".*standard_(\d+)_(\w+)_results\.csv", f)
    if not m:
        continue
    N = int(m.group(1))
    kernel = m.group(2)
...
# Error bars: min..max after trimming
...
for i, k in enumerate(kernels):
    ...
    for bar, val, std in zip(bars, heights, stds):
        ax.text(bar.get_x() + bar.get_width()/2,
                bar.get_height(),
                f"{val:.2f} ± {2*std:.2f}",   # mean ± 2*std in µs
...
ax.set_title("Kernel runtimes (outliers removed, min/max error bars)")
ax.legend()
plt.tight_layout()
plt.show()
